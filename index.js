...
app.post('/api/auth/send-otp',async(req,res)=>{
  try{
    ...
    await pool.query('INSERT INTO otp_codes (email,code,data,expires_at) VALUES ($1,$2,$3,$4)',
      [email,code,JSON.stringify({first_name,last_name,email,password,phone}),expires]);
    let mailSent = false;
    if (mailer) {
      try {
        await mailer.sendMail({
          from:'"100TH Store" <'+process.env.MAIL_USER+'>',
          to:email,
          subject:'Your 100TH verification code',
          html:`<div style="font-family:sans-serif;max-width:400px;margin:0 auto;padding:32px;background:#060606;color:#fff;border:1px solid #222;"><div style="font-size:28px;font-weight:700;letter-spacing:0.12em;margin-bottom:8px;">100<span style="color:#08f7f7;">TH</span></div><p style="color:rgba(255,255,255,0.6);margin-bottom:24px;">Your verification code:</p><div style="font-size:48px;font-weight:900;letter-spacing:0.2em;color:#08f7f7;text-align:center;padding:20px;background:rgba(8,247,247,0.06);border:1px solid rgba(8,247,247,0.2);">${code}</div><p style="color:rgba(255,255,255,0.4);font-size:12px;margin-top:20px;">This code expires in 10 minutes.</p></div>`
        });
        mailSent = true;
      } catch(mailErr) { console.error('Mail error:',mailErr.message); }
    }
    if (!mailSent) {
      console.log('[OTP] '+email+' -> '+code+' (email disabled: MAIL_USER/MAIL_PASS not set)');
      return res.json({message:'OTP sent to '+email, _dev_code: code});
    }
...
app.post('/api/auth/verify-otp',async(req,res)=>{
  try{
    ...
    const result=await pool.query('SELECT * FROM otp_codes WHERE email=$1 AND code=$2 AND used=false AND expires_at>NOW()',[email,code]);
    if(result.rows.length===0) return res.status(400).json({error:'Invalid or expired code'});
    const{first_name,last_name,password,phone}=result.rows[0].data;
    await pool.query('UPDATE otp_codes SET used=true WHERE id=$1',[result.rows[0].id]);
    const existing=await pool.query('SELECT id FROM users WHERE email=$1',[email]);
    if(existing.rows.length>0) return res.status(400).json({error:'Email already registered'});
    const hash=await bcrypt.hash(password,10);
    const user=await pool.query('INSERT INTO users (first_name,last_name,email,password,phone) VALUES ($1,$2,$3,$4,$5) RETURNING id,first_name,last_name,email,is_admin',
      [first_name,last_name,email,hash,phone||null]);
    const token=jwt.sign(user.rows[0],process.env.JWT_SECRET||'100th_secret',{expiresIn:'30d'});
    res.json({token,user:user.rows[0]});
  }catch(err){res.status(500).json({error:err.message});}
});

app.post('/api/auth/register',async(req,res)=>{
  // legacy fallback — just redirect to send-otp
  res.status(400).json({error:'Please use the OTP registration flow'});
});
...
app.patch('/api/admin/orders/:ref', adminAuth, async(req,res)=>{
  try{
    const{status, payment_status, delivery_tracking}=req.body;
    const fields=[]; const vals=[];
    if(status){fields.push('status=$'+( vals.push(status)));} 
    if(payment_status){fields.push('payment_status=$'+(vals.push(payment_status)));}
    if(delivery_tracking!==undefined){fields.push('delivery_tracking=$'+(vals.push(delivery_tracking)));}
    if(!fields.length) return res.status(400).json({error:'Nothing to update'});
    vals.push(req.params.ref);
    await pool.query('UPDATE orders SET '+fields.join(',')+' WHERE order_ref=$'+vals.length, vals);
    res.json({success:true});
  }catch(err){res.status(500).json({error:err.message});}
});
...
app.get('/api/admin/customers', adminAuth, async(req,res)=>{
  try{
    const result=await pool.query('SELECT id,first_name,last_name,email,phone,is_admin,created_at,(SELECT COUNT(*) FROM orders WHERE user_id=users.id) as order_count FROM users ORDER BY created_at DESC');
    res.json(result.rows);
  }catch(err){res.status(500).json({error:err.message});}
});
